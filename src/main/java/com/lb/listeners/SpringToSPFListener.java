...
public class SpringToSPFListener extends PropertyListenerAdapter implements PublisherExtension {
    ...
    public SpringToSPFListener(Config conf, JPF jpf) {
...
//     }
    ...
    private void printMethodSummary(PrintWriter pw, MethodSummary methodSummary) {
        ...
        if (pathConditions.size() > 0) {
                ...
                while (st2.hasMoreTokens()) {
                    ...
                    if (pc.contains(token)) {
                        ...
                        if (actualType == Types.T_INT || actualType == Types.T_FLOAT || actualType == Types.T_LONG
                            ...
                            Pattern referenceTypePattern = Pattern.compile("(.*)[.](.*)@(.*)");
                            Matcher referenceMatcher = referenceTypePattern.matcher(actualValue);

                            Pattern fieldPattern = Pattern.compile("(.*)SYMREF\\.(?<reference>[^\\.]*)(.*)\\[");
                            Matcher fieldMatcher = fieldPattern.matcher(pc);

                            if(referenceMatcher.find() && fieldMatcher.find()) {
                                String referenceType = referenceMatcher.group(2);
                                String field = fieldMatcher.group(2);

                                testCase = testCase + referenceType + "." + field + "=" + val + ",";
                            } else {
                                testCase = "No TestCase could be generated";
                            }
                        } else
            ...
            currentTestCases = allTestCases.substring(1);
        } else {
            pw.println("No path conditions for " + methodSummary.getMethodName() + "(" + methodSummary.getArgValues()
                    + ")");
            currentTestCases = "No path conditions for " + methodSummary.getMethodName() + "(" + methodSummary.getArgValues()
                    + ")";
        }
    }
    ...
    public void publishFinished(Publisher publisher) {
        ...
        while (it.hasNext()) {
            Map.Entry me = (Map.Entry) it.next();
            SpringToSPFListener.MethodSummary methodSummary = (SpringToSPFListener.MethodSummary) me.getValue();
            printMethodSummary(pw, methodSummary);
        }
    }

    public String getCurrentTestCases() {
